        ...
        for a, num_moved in enumerate(actionSpace):
            if num_moved > num_cars1 or -num_moved > num_cars2 or \
                ...
                for s_next in range(num_states):
                    ...
                    if num_moved>0:
                        #This if condition ensures that num of car moves is positive
                        R[s,a,s_next] = reward1 + reward2 - moving_cost * abs(num_moved-1)
                    #added a -1 in the num of cars moved as the employee can move one car free of cost
                    else:
                        R[s,a,s_next] = reward1 + reward2 - moving_cost * abs(num_moved)
                        #If this num is -ve or zero then employee cant move car for free

                    #Conditions for parking costs
                    if morning_cars2>parking_limit:
                        R[s,a,s_next] = R[s,a,s_next] + parking_cost

                    if morning_cars1>parking_limit:
                        R[s,a,s_next] = R[s,a,s_next] + parking_cost
    return P, R
